...
if (isset($input['first_name']) && 
	...
	if(1){
		...
		if($stmt = $con->prepare($getHijriQuery)){
			...
			$stmt->execute();
			//$stmt->bind_result($hijri_year, $hijri_eng, $ramadan_last);
			//$stmt->fetch();
			...
			$insertHQuery  = "INSERT INTO member_registration (member_id, hijri_year, registration_date, ip_address, prepaid) VALUES (?,?,?,?,?)";
			if($hstmt = $con->prepare($insertHQuery)){
				$hstmt->bind_param("sssss", $member_id, $hijri_eng, $registration_date, $ip_address, $prepaid);
				$hstmt->execute();
				//$response["status"] = 0;
				//$response["message"] = "User created";
				$i++;
			...
			if ($registration_date < $sixty_before_date) {
				...
				if($nstmt = $con->prepare($getNextHijriQuery)){
					...
					$nstmt->execute();
					$nstmt->bind_result($hijri_year, $hijri_eng, $shawwal_first);


					$insertHQuery  = "INSERT INTO member_registration (member_id, hijri_year, registration_date, ip_address, prepaid) VALUES (?,?,?,?,?)";
					if($hstmt = $con->prepare($insertHQuery)){
						$hstmt->bind_param("sssss", $member_id, $hijri_eng, $shawwal_first, $ip_address, $prepaid);
						$hstmt->execute();
						//$response["status"] = 0;
						//$response["message"] = "User created";
						$i++;
		...
		while ($i < $num_years)
		{
			...
			if($nstmt = $con->prepare($getNextHijriQuery)){
				...
				if($stmt = $con->prepare($insertHQuery)){
					$stmt->bind_param("sssss", $member_id, $hijri_eng, $shawwal_first, $ip_address, $prepaid);
					$stmt->execute();
					//$response["status"] = 0;
					//$response["message"] = "User created";
					$stmt->close();
